...
import heapq
import logging
from typing import Dict, Optional, Tuple, List, Any, Type, Union
                ...
                if (nx, ny) != goal:
                    ...
                    if isinstance(actor, Hero):
                        ...
                        if state and any(
                            ...
                            occupied = True
                        if any(a.x == nx and a.y == ny for a in getattr(self.world, "player_armies", [])):
                            occupied = True
                    else:
                        if (
        ...
        for enemy in list(heroes):
            ...
            if tile.building and isinstance(tile.building, Town) and not tile.building.passable:
                ...
                if enemy.ap >= step_cost:
                    ...
                    econ_b = getattr(self, "_econ_building_map", {}).get(tile.building)
                    try:
                        tile.capture(enemy, 1, econ_state, econ_b)
                    except Exception:
                        pass
                continue
